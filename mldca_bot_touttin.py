...
═══════════════════════════════════════════════════════════════════
THREE-CATEGORY EXECUTION STRATEGY
═══════════════════════════════════════════════════════════════════

Each symbol is assigned an execution category, configured in
SYMBOL_CATEGORIES:

  CATEGORY 1 (BUY ONLY):
    - Buys on 2d low triggers (same as the original engine).
    - No sell behavior.

  CATEGORY 2 (BUY AND SELL):
    - Buys on 2d low triggers (same as Category 1).
    - ALSO sells (closes long) on 2d high triggers when there is
      an open long position for that symbol.

  CATEGORY 3 (SELL ONLY):
    - Does NOT buy.
    - Sells (closes long) on 2d high triggers when there is an open
      long position for that symbol.

Symbol assignments:
  - Category 1: All crypto symbols and INDA
  - Category 2: USOIL and uranium (URNM)
  - Category 3: All other symbols

SELL (CLOSE LONG) SEMANTICS:
  - A sell trigger requires the most recently closed 1-minute
    candle's high to be STRICTLY GREATER THAN the rolling 2d high
    (excluding the current candle) — mirroring the strict new-low
    semantics of the buy trigger.
  - Sells only fire when there is an open long position for that
    symbol (tracked via open order records and filled test orders).
  - Sells close the ENTIRE open long position (reduce-only close
    long, side=4 on MEXC).
  - Sells do NOT use budget — the budget accounting applies only to
    buys. A sell does not deduct from or add to the running budget.
  - Any attempted sell order below MOS is stacked before firing:
    sell sizes accumulate in a per-symbol sell accumulator, and
    only fire once the contract-equivalent clears the exchange's
    minimum order size. This mirrors the buy accumulator behavior.
  - Sell sizing: the sell amount is the full size of the open long
    position (in contracts), converted to USD at the triggering
    candle's high price for MOS comparison purposes.
...
Per-symbol running budget:
  ...
  - Every time a real BUY order is placed for a symbol, its USD
    amount is subtracted from that symbol's running budget. The
    budget can go negative. SELL orders do NOT affect the budget.
...
EVERY symbol, including FAILED ones — see FAILED SYMBOLS below):
  ...
  3. If the closed candle's low is STRICTLY LESS THAN that
     reference low, it is a BUY TRIGGER.
  4. For BUY-capable symbols (Category 1 and 2) only: a buy
     trigger first updates BudgetR (see BUDGET-R below), then adds
     an order-size amount (see ORDER SIZING below) to the pending
     buy accumulator, and if the accumulator's contract-equivalent
     at the triggering candle's low is >= the exchange's minimum
     order size, a real limit LONG is placed at exactly that low
     price for the full accumulated amount. On a SUCCESSFUL
     placement, the accumulator resets to 0 and the placed amount
     is subtracted from the running budget. On a FAILED placement
     (rejected by the exchange), the accumulator is left untouched
     — i.e. it retains the order-size amount that was just added
     for this trigger, so a failed order's size is carried forward
     into the accumulation rather than discarded (see FAILED-ORDER
     CARRY-FORWARD below).
  5. For SELL-capable symbols (Category 2 and 3) with an open long
     position: if the closed candle's high is STRICTLY GREATER
     THAN the rolling 2d high (excluding the current candle), it
     is a SELL TRIGGER. The sell attempts to close the entire open
     long position. If the sell size is below MOS, it is stacked
     in the sell accumulator and carried forward until it clears
     MOS.
  6. For FAILED symbols: triggers are evaluated and recorded (so
     they can be marked on the chart) but NO budget accrual, NO
...
A buy trigger requires the most recently closed candle's low to be
a genuine new low relative to the rest of the reference window —
that is, STRICTLY LOWER than every OTHER candle's low in that
window — rather than merely being less-than-or-equal-to the window
minimum (which, since the current candle is itself a member of that
window, would be trivially satisfied whenever the current candle
ties or sets the minimum, including tying itself).
...
Sell triggers mirror this exactly: a sell trigger requires the
most recently closed candle's high to be STRICTLY GREATER THAN the
rolling 2d high computed over the window EXCLUDING the current
candle, via rolling_high(..., exclude_latest=True), with the
condition:
    sell_triggered = candle_high > ref_high
...
For BUY-capable symbols (Category 1 and 2), each buy trigger adds
an order-size amount to the pending buy accumulator, computed as:
...
SELL sizing is separate and does NOT use this formula. Sells close
the entire open long position; see THREE-CATEGORY EXECUTION
STRATEGY above.
...
order_size_usd (see ORDER SIZING above) is added to the buy
accumulator BEFORE a real order placement is attempted. If that
placement then FAILS (rejected by the exchange, or any other
placement failure), the accumulator is left exactly as it was after
the addition — i.e. it is NOT reset to zero on failure, only on a
SUCCESSFUL placement. This means a failed order's order_size_usd is
automatically carried forward into the accumulation for the next
trigger, rather than being discarded. No separate "re-add on
failure" step exists or is needed; this falls directly out of the
accumulator only ever being reset on the success path.

The same carry-forward rule applies to the sell accumulator: a
failed sell leaves the sell accumulator intact so the size carries
forward into the next sell trigger.
...
Budget accrual applies ONLY to BUY-capable symbols (Category 1 and
2). Category 3 symbols are sell-only and never accrue budget,
never carry a budget, and never compute order sizes — their budget
...
OHLC candles (~960 candles). The chart marks:
  - Whichever reference low is currently active (2d or 9d, based on
    ...
    way.
  - For sell-capable symbols, the rolling 2d high is also drawn as
    a dashed horizontal threshold line (in a distinct color), so
    the sell trigger level is visible.
  - Every BUY TRIGGER (candle low strictly less than the reference
    low, excluding the current candle — see STRICT NEW-LOW
    SEMANTICS) as a small tick marker on the price axis at that
    candle's time/price.
  - Every SELL TRIGGER (candle high strictly greater than the
    rolling 2d high, excluding the current candle) as a small tick
    marker, in a distinct color.
  - Every REAL ORDER placed for that symbol (never happens for
    failed symbols) as a filled circle marker at its fire
    time/price. Buy and sell orders are drawn in distinct colors.
...
each abbreviated column header to its full name. Columns, in order:
  ...
  Sym     Symbol code. A failed symbol has "[F]" appended directly
          ...
          symbol code.
  Cat     Execution category (1, 2, or 3) for this symbol — see
          THREE-CATEGORY EXECUTION STRATEGY. NOT summed in Totals;
          left blank.
  ...
  Acc     Current buy accumulator value in USD. NOT summed in
          Totals — this is a per-symbol pending-order figure rather
          than a realized portfolio quantity, and summing it would
          read as "total money about to be spent," which is not
          what the Totals row is for; left blank. (If a running
          total of pending accumulation is wanted, it can be added
          as an explicit, separately-labeled figure on request.)
  AccS    Current sell accumulator value in contracts. NOT summed
          in Totals; left blank.
...
#
# Category 1: 2d low buy only (all crypto + INDA)
# Category 2: 2d low buy AND 2d high sell if open long (USOIL, URNM)
# Category 3: 2d high sell only if open long (all others)
#
# See THREE-CATEGORY EXECUTION STRATEGY in the module docstring.

CATEGORY_1 = {
    "BTC_USDT", "ETH_USDT", "SOL_USDT", "XRP_USDT", "TRX_USDT",
    "BNB_USDT", "NGAS_USDT", "XPD_USDT", "XAU_USDT", "MSTRSTOCK_USDT",
    "UNITREE_USDT", "SPX500_USDT", "EWJ_USDT", "EWY_USDT",
    "HK0700_USDT", "EWT_USDT", "SMH_USDT", "COPPER_USDT",
    "BKRSTOCK_USDT", "INDA_USDT",
}

CATEGORY_2 = {
    "USOIL_USDT", "URNM_USDT",
}

CATEGORY_3 = {
    sym for sym in SYMBOLS
    if sym not in CATEGORY_1 and sym not in CATEGORY_2
}

SYMBOL_CATEGORIES: Dict[str, int] = {}
for _s in SYMBOLS:
    if _s in CATEGORY_1:
        SYMBOL_CATEGORIES[_s] = 1
    elif _s in CATEGORY_2:
        SYMBOL_CATEGORIES[_s] = 2
    else:
        SYMBOL_CATEGORIES[_s] = 3


def get_category(sym: str) -> int:
    return SYMBOL_CATEGORIES.get(sym, 1)


def is_buy_capable(sym: str) -> bool:
    return get_category(sym) in (1, 2)


def is_sell_capable(sym: str) -> bool:
    return get_category(sym) in (2, 3)
...
TABLE_COLUMNS: List[Tuple[str, str]] = [
    ...
    ("Acc",      "Buy Accumulator (USD)"),
    ("AccS",     "Sell Accumulator (Contracts)"),
...
def get_sell_trigger_count(sym: str) -> int:
    return int(STATE_DATA["sell_trigger_count"].get(sym, 0))
...
# ── open long position tracking ──────────────────────────────────────────────

def get_open_long_contracts(sym: str) -> float:
    return float(STATE_DATA["open_long_contracts"].get(sym, 0.0))


def add_open_long(sym: str, contracts: float, price: float):
    with _STATE_DATA_LOCK:
        prev = get_open_long_contracts(sym)
        new = prev + contracts
        STATE_DATA["open_long_contracts"][sym] = new
        STATE_DATA["open_long_entry"][sym] = price
        _persist()
        log.info(
            f"[{sym}] open long updated: {prev} + {contracts} = {new} contracts"
        )


def reduce_open_long(sym: str, contracts: float) -> float:
    with _STATE_DATA_LOCK:
        prev = get_open_long_contracts(sym)
        new = max(0.0, prev - contracts)
        STATE_DATA["open_long_contracts"][sym] = new
        if new <= 0:
            STATE_DATA["open_long_entry"][sym] = None
        _persist()
        log.info(
            f"[{sym}] open long reduced: {prev} - {contracts} = {new} contracts"
        )
        return new
...
def get_sell_accumulator(sym: str) -> float:
    return float(STATE_DATA["sell_accumulator"].get(sym, 0.0))


def add_to_sell_accumulator(sym: str, contracts: float) -> float:
    with _STATE_DATA_LOCK:
        prev = float(STATE_DATA["sell_accumulator"].get(sym, 0.0))
        new = prev + contracts
        STATE_DATA["sell_accumulator"][sym] = new
        _persist()
        return new


def deduct_sell_accumulator(sym: str, contracts: float):
    with _STATE_DATA_LOCK:
        prev = float(STATE_DATA["sell_accumulator"].get(sym, 0.0))
        new = max(0.0, prev - contracts)
        STATE_DATA["sell_accumulator"][sym] = new
...
def record_trigger_marker(
    ...
    with _STATE_DATA_LOCK:
        ...
        if kind == "sell":
            STATE_DATA["sell_triggers"].append(entry)
        else:
            STATE_DATA["triggers"].append(entry)
        ...
        STATE_DATA["sell_triggers"] = [
            t for t in STATE_DATA["sell_triggers"]
            if _safe_ts(t.get("candle_time")) is None
            or _safe_ts(t.get("candle_time")) >= cutoff
        ]
...
def place_close_long(
    sym: str,
    limit_price: float,
    contracts: float
) -> Optional[str]:
    """Places a limit order to close (sell) a long position.
    On MEXC, closing a long is a sell with side=4 (close long),
    reduceOnly=true. Returns the order id, "SKIP" if below minimum
    size, or None on failure."""
    if contracts < _mos(sym):
        log.warning(
            f"[{sym}] close-long size {contracts} < min "
            f"{_mos(sym)} — order skipped (will stack)"
        )
        return "SKIP"

    return _place_close_long_contracts(sym, limit_price, contracts)


def _place_close_long_contracts(
    sym: str,
    limit_price: float,
    vol: float
) -> Optional[str]:
    ...
    if not r.get("success"):
        log.error(f"[{sym}] close-long order rejected: {r}")
        return None
...
class MinuteBuffer:
    ...
    def rolling_high(
        ...
        omitted from the set being scanned, so the result reflects
        only PRIOR candles in the window. This is required for
        strict new-high sell trigger evaluation — see STRICT
        NEW-LOW / NEW-HIGH SEMANTICS in the module docstring —
        where the current candle must be compared against the rest
        of the window rather than against a set that trivially
        includes itself.
        """
        with self.lock:
            if not self.bars:
...
def evaluate_buy_trigger(sym: str, now_utc: datetime.datetime):
    ...
    log.info(
        ...
        f"buy_trigger={triggered}"
    )

    return candle_dt, candle_low, ref_label, triggered


def evaluate_sell_trigger(sym: str, now_utc: datetime.datetime):
    """Evaluates the 2d-high sell condition for SELL-capable
    symbols with an open long position. Returns
    (candle_dt, candle_high, triggered) or None if not evaluable."""
    if not is_sell_capable(sym):
        return None

    open_contracts = get_open_long_contracts(sym)
    if open_contracts <= 0:
        return None
    ...
    candle_dt = datetime.datetime.fromtimestamp(latest["t"], tz=UTC)
    candle_high = latest["h"]

    ref_high = buf.rolling_high(ROLL_MINUTES_SHORT, exclude_latest=True)

    if ref_high is None:
        return None

    triggered = candle_high > ref_high

    log.info(
        f"[{sym}] sell check {candle_dt.isoformat()}: "
        f"high={candle_high:.4f} "
        f"2dHigh(prior)={ref_high:.4f} "
        f"open_long={open_contracts} "
        f"sell_trigger={triggered}"
    )

    return candle_dt, candle_high, triggered


def process_symbol_minute(sym: str, now_utc: datetime.datetime):
    failed = is_failed(sym)
    category = get_category(sym)
    buy_capable = is_buy_capable(sym)
    sell_capable = is_sell_capable(sym)

    # ── budget accrual (buy-capable symbols only) ──
    if not failed and buy_capable:
        today = now_utc.date()
        accrue_daily_budget_if_due(sym, today)

    # ── buy trigger evaluation ──
    if buy_capable:
        result = evaluate_buy_trigger(sym, now_utc)

        if result is not None:
            candle_dt, candle_low, ref_label, triggered = result

            if triggered:
                record_trigger_marker(sym, candle_dt, candle_low, ref_label, kind="buy")
                record_trigger_stat(sym)

                # BudgetR is updated for EVERY trigger, failed symbols
                # included, per the BUDGET-R rule — see module
                # docstring. This must happen BEFORE the order-sizing
                ...
                if failed:
                    log.info(
                        f"[{sym}] BUY TRIGGER (failed symbol, "
                        f"marker-only, no accumulation) "
                    ...
                    log.info(
                        f"[{sym}] BUY TRIGGER — accumulator now "
                        f"${pending:.2f} "
                        f"(order_size=${order_size_usd:.4f} added "
                        f"this trigger) @ price={candle_low:.4f}"
                    )
                    ...
                    if live_budget < 0:
                        log.info(
                            f"[{sym}] budget is negative "
                            f"(${live_budget:.2f}) — stacking "
                            f"${order_size_usd:.4f} on accumulator "
                            f"(now ${pending:.2f}) instead of firing"
                        )
                    else:
                        attempt_usd = pending

                        # Positive budget safety cap
                        if pending > live_budget:
                            attempt_usd = live_budget
                            log.info(
                                f"[{sym}] pending (${pending:.2f}) "
                                f"exceeds positive budget "
                                f"(${live_budget:.2f}) — capping "
                                f"attempt to available budget "
                                f"(${attempt_usd:.2f})"
                            )
                        ...
                        if vol_at_price < _mos(sym):
                            log.info(
                                f"[{sym}] attempt size "
                                f"${attempt_usd:.2f} "
                                f"(vol={vol_at_price}) below min "
                                f"order size ({_mos(sym)} contracts "
                                f"@ {candle_low:.4f}) — stacking, "
                                "no order placed"
                            )
                        else:
                            log.info(
                                f"[{sym}] accumulator attempt "
                                f"${attempt_usd:.2f} reaches min "
                                f"order size — attempting limit "
                                f"LONG {vol_at_price} contracts "
                                f"@ {candle_low:.4f} "
                            ...
                            if oid == "SKIP" or oid is None:
                                record_attempt_stat(
                                    sym, candle_low, success=False
                                )
                                record_lifetime_order_outcome(
                                    sym, success=False
                                )
                                # Accumulator is deliberately NOT
                                # reset here — it retains the amount
                                # that was just added above, so this
                                # failed attempt's amount carries
                                # forward into the next trigger's
                                # accumulation.
                                if oid == "SKIP":
                                    log.warning(
                                        f"[{sym}] fire skipped by "
                                        "place_long despite passing "
                                        "pre-check — leaving "
                                        "accumulator intact "
                                        f"(carries forward "
                                        f"${order_size_usd:.4f} "
                                    ...
                                    log.error(
                                        f"[{sym}] minute-trigger order "
                                        "rejected by MEXC — leaving "
                                        "accumulator intact, will "
                                ...
                                record_attempt_stat(
                                    sym, candle_low, success=True,
                                    usd_if_success=actual_usd
                                )
                                record_lifetime_order_outcome(
                                    sym, success=True
                                )
                                ...
                                add_open_long(sym, vol_at_price, candle_low)
    ...
    if sell_capable:
        sell_result = evaluate_sell_trigger(sym, now_utc)

        if sell_result is not None:
            sell_dt, candle_high, sell_triggered = sell_result

            if sell_triggered:
                record_trigger_marker(
                    sym, sell_dt, candle_high, "2d", kind="sell"
                )

                with _STATE_DATA_LOCK:
                    STATE_DATA["sell_trigger_count"][sym] = (
                        get_sell_trigger_count(sym) + 1
                    )
                    _persist()

                open_contracts = get_open_long_contracts(sym)
                log.info(
                    f"[{sym}] SELL TRIGGER @ high={candle_high:.4f} "
                    f"(open long: {open_contracts} contracts)"
                )

                if failed:
                    log.info(
                        f"[{sym}] SELL TRIGGER (failed symbol, "
                        "marker-only, no order attempted)"
                    )
                else:
                    # Add the full open position size to the sell
                    # accumulator. Per the requirement, any attempted
                    # sell below MOS is stacked before firing.
                    pending_sell = add_to_sell_accumulator(
                        sym, open_contracts
                    )

                    log.info(
                        f"[{sym}] sell accumulator now "
                        f"{pending_sell:.4f} contracts "
                        f"(added {open_contracts:.4f} this trigger)"
                    )

                    vol_at_price = pending_sell
                    cs = specs.get(sym, {}).get("cs", 1.0)
                    actual_usd = vol_at_price * cs * candle_high

                    if vol_at_price < _mos(sym):
                        log.info(
                            f"[{sym}] sell attempt "
                            f"{vol_at_price:.4f} contracts below min "
                            f"order size ({_mos(sym)} contracts) — "
                            "stacking, no sell order placed"
                        )
                    else:
                        log.info(
                            f"[{sym}] sell accumulator "
                            f"{vol_at_price:.4f} contracts reaches "
                            f"min order size — attempting limit "
                            f"CLOSE-LONG @ {candle_high:.4f} "
                            f"(notional: ${actual_usd:.2f})"
                        )

                        oid = place_close_long(
                            sym, candle_high, vol_at_price
                        )

                        if oid == "SKIP" or oid is None:
                            record_attempt_stat(
                                sym, candle_high, success=False
                            )
                            record_lifetime_order_outcome(
                                sym, success=False
                            )
                            # Sell accumulator deliberately NOT reset
                            # on failure — carries forward.
                            if oid == "SKIP":
                                log.warning(
                                    f"[{sym}] sell skipped by "
                                    "place_close_long despite passing "
                                    "pre-check — leaving sell "
                                    "accumulator intact "
                                    f"(carries forward "
                                    f"{open_contracts:.4f} contracts)"
                                )
                            else:
                                log.error(
                                    f"[{sym}] sell order rejected by "
                                    "MEXC — leaving sell accumulator "
                                    "intact, will retry on next sell "
                                    f"trigger (carries forward "
                                    f"{open_contracts:.4f} contracts)"
                                )
                        else:
                            record_attempt_stat(
                                sym, candle_high, success=True,
                                usd_if_success=actual_usd
                            )
                            record_lifetime_order_outcome(
                                sym, success=True
                            ...
                            deduct_sell_accumulator(sym, vol_at_price)
                            reduce_open_long(sym, vol_at_price)

                            record_order({
                                "symbol": sym,
                                "timestamp": now_utc.isoformat(),
                                "candle_time": sell_dt.isoformat(),
                                "order_id": oid,
                                "limit_price": candle_high,
                                "usd": actual_usd,
                                "vol": vol_at_price,
                                "reference_window": "2d",
                                "kind": "sell",
...
def _close_test_order(pending: Dict):
    ...
    try:
        if is_filled(sym, oid):
            ...
            add_open_long(sym, pending["vol"], pending["limit_price"])
            return
...
def render_overview_table_html(now_utc: datetime.datetime) -> str:
    ...
    for sym in SYMBOLS:
        ...
        acc_s = get_sell_accumulator(sym)
...
def render_symbol_chart_svg(sym: str) -> str:
    ...
    ref_high = None
    if is_sell_capable(sym):
        ref_high = buf.rolling_high(ROLL_MINUTES_SHORT)
    ...
    if ref_high is not None:
        ...
        svg.append(
            f'<line x1="{CHART_MARGIN_L}" y1="{ry_h:.1f}" '
            f'x2="{CHART_W - CHART_MARGIN_R}" y2="{ry_h:.1f}" '
            f'stroke="#008800" stroke-width="1.2" '
            f'stroke-dasharray="6,3"/>'
        )

        svg.append(
            f'<text x="{CHART_W - CHART_MARGIN_R - 4}" y="{ry_h - 4:.1f}" '
            f'font-family="Courier New" font-size="10" '
            f'fill="#008800" text-anchor="end">'
            f'{_xml_escape(f"2d high threshold: {ref_high:,.4f}")}</text>'
        )
    ...
    for t in sell_markers:
        ...
        svg.append(
            f'<rect x="{tx - sz:.1f}" y="{ty - sz:.1f}" '
            f'width="{sz * 2:.1f}" height="{sz * 2:.1f}" '
            f'fill="none" stroke="#e07b00" stroke-width="1.3"/>'
        )
    ...
    # sell trigger legend
    svg.append(
        f'<rect x="{CHART_MARGIN_L + 110}" y="{legend_y - 4}" '
        f'width="8" height="8" '
        f'fill="none" stroke="#e07b00" stroke-width="1.3"/>'
    )
    svg.append(
        f'<text x="{CHART_MARGIN_L + 124}" y="{legend_y + 4}" '
        f'font-family="Courier New" font-size="10" fill="#555">'
        f'sell trigger</text>'
    )
    ...
    # buy order legend
    svg.append(
        f'<circle cx="{CHART_MARGIN_L + 230}" cy="{legend_y}" r="4" '
        f'fill="#0044cc" stroke="#fff" stroke-width="1"/>'
    )
    svg.append(
        f'<text x="{CHART_MARGIN_L + 240}" y="{legend_y + 4}" '
        f'font-family="Courier New" font-size="10" fill="#555">'
        f'buy order</text>'
    )
    ...
    # sell order legend
    svg.append(
        f'<circle cx="{CHART_MARGIN_L + 330}" cy="{legend_y}" r="4" '
        f'fill="#e07b00" stroke="#fff" stroke-width="1"/>'
    )
    svg.append(
        f'<text x="{CHART_MARGIN_L + 340}" y="{legend_y + 4}" '
        f'font-family="Courier New" font-size="10" fill="#555">'
        f'sell order</text>'
    )
    ...
    def do_GET(self):
        if self.path.startswith("/chart/") and self.path.endswith(".svg"):
            ...
            body = json.dumps(
                {
                    ...
                    "sell_accumulator": STATE_DATA["sell_accumulator"],
                    "open_long_contracts": STATE_DATA["open_long_contracts"],
                    ...
                    "categories": {sym: get_category(sym) for sym in SYMBOLS},
                },
                indent=2
            ).encode("utf-8")
            ...
            body = json.dumps(
                {
                    sym: {
                        ...
                        "buy_capable": is_buy_capable(sym),
                        "sell_capable": is_sell_capable(sym),
                        "open_long_contracts": get_open_long_contracts(sym),
                        "open_long_entry": STATE_DATA["open_long_entry"].get(sym),
                        "buy_accumulator_usd": get_accumulator(sym),
                        "sell_accumulator_contracts": get_sell_accumulator(sym),
                    }
                    for sym in SYMBOLS
                },
                indent=2
            ...
            html_doc = (
                ...
                " · "
                "<a href='/positions.json'>positions &amp; categories</a>"
                " · "
