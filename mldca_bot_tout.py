...
═══════════════════════════════════════════════════════════════════
SYMBOL EXECUTION MODE (TYPE 1 vs TYPE 2)
═══════════════════════════════════════════════════════════════════

Every symbol is assigned an execution MODE, either 1 or 2, in the
SYMBOL_MODES map (see SYMBOL CONFIGURATION below). The mode inverts
BOTH the direction of the trigger condition AND the direction of
the order placed:

  MODE 1 — "long / buy-the-dip" (the bot's original behavior):
    - Trigger condition: the most recently closed 1-minute candle's
      LOW must be a STRICT new low versus the reference window
      (2d or 9d, per the budget-sign rule).
    - Action: opens a limit LONG at the triggering candle's low.
    - Budget: a successful placement SUBTRACTS the placed USD amount
      from the symbol's running budget (see BUDGET ACCRUAL / budget
      spending below).

  MODE 2 — "close / sell-the-rip":
    - Trigger condition: the most recently closed 1-minute candle's
      HIGH must be a STRICT new high versus the reference window
      (2d or 9d, per the budget-sign rule). This is the exact
      mirror of mode 1's trigger: where mode 1 compares the current
      low against the prior-window low, mode 2 compares the current
      high against the prior-window high.
    - Action: opens a limit SHORT (i.e. CLOSES long exposure) at
      the triggering candle's high.
    - Budget: a successful placement does NOT deduct from the
      symbol's running budget. Closing positions is the unwind
      side of the strategy; it does not consume the dip-buying
      budget. (BudgetR and the accumulator are still tracked and
      updated identically for mode-2 symbols — see BUDGET-R and
      ORDER SIZING — so the sizing math and per-trigger accounting
      remain symmetric across modes; only the budget deduction on
      successful placement is suppressed for mode 2.)

The reference-window choice (2d when budget >= 0, 9d when
budget < 0) is IDENTICAL for both modes — it is driven purely by
the symbol's running budget sign, exactly as before, and is not
affected by the mode.

All other behavior — minute-trigger engine cadence, buffer
management, contribution weighting, BudgetR, accumulator
semantics, failed-symbol handling, charts, overview table, daily
report — is unchanged between modes, except where explicitly noted
above (trigger direction, order side, and the budget-deduction
suppression for mode 2).
...
Per-symbol running budget:
  - Starts at $0.
  - At every UTC midnight, an accrual of (2 x that symbol's
  ...
  - Every time a real order is placed for a MODE-1 symbol, its USD
    amount is subtracted from that symbol's running budget. The
    budget can go negative. MODE-2 symbols never deduct from their
    budget on placement (see SYMBOL EXECUTION MODE above).
...
EVERY symbol, including FAILED ones — see FAILED SYMBOLS below):
  ...
  2. Choose the reference window based on the symbol's CURRENT
     running budget at the moment of the check:
       - budget >= 0  -> reference is the rolling 2-day low/high
       - budget <  0  -> reference is the rolling 9-day low/high
  ...
  3. MODE 1: if the closed candle's low is STRICTLY LESS THAN that
     reference low, it is a TRIGGER.
     MODE 2: if the closed candle's high is STRICTLY GREATER THAN
     that reference high, it is a TRIGGER.
  4. For NON-FAILED symbols only: a trigger first updates BudgetR
     ...
     price is >= the exchange's minimum order size, a real limit
     order is placed at exactly that price for the full accumulated
     amount — a LONG for mode 1, a SHORT for mode 2. On a
     SUCCESSFUL placement, the accumulator resets to 0 and (for
     mode 1 only) the placed amount is subtracted from the running
     budget. On a FAILED placement (rejected by the exchange), the
...
Rolling 1-minute OHLC candle buffer (per symbol):
  ...
  - Updated every minute by fetching only the most recently closed
    ...
    failed status.
  - Powers both the trigger-reference lows/highs AND the
    15m-resampled 10-day chart (see CHARTS below), AND the overview
    table's dollar-denominated MOS column (see MAIN OVERVIEW TABLE
    below), which reuses each symbol's most recently closed 1-minute
    close already resident in the buffer rather than issuing a live
    mark-price fetch, so rendering the table adds no network calls.

═══════════════════════════════════════════════════════════════════
STRICT NEW-LOW / NEW-HIGH SEMANTICS
═══════════════════════════════════════════════════════════════════

A trigger requires the most recently closed candle's low (mode 1)
or high (mode 2) to be a genuine new extreme relative to the rest
of the reference window — that is, STRICTLY LOWER (mode 1) or
STRICTLY HIGHER (mode 2) than every OTHER candle's corresponding
extreme in that window — rather than merely being <= / >= the
window's extreme (which, since the current candle is itself a
member of that window, would be trivially satisfied whenever the
current candle ties or sets the extreme, including tying itself).

To enforce this:
  - The reference low (mode 1) or high (mode 2), 2d or 9d per the
    budget-sign rule above, is computed over the window EXCLUDING
    the current candle, via rolling_low(..., exclude_latest=True) /
    rolling_high(..., exclude_latest=True).
  - The trigger condition uses strict inequality:
        mode 1:  triggered = candle_low  < ref_low
        mode 2:  triggered = candle_high > ref_high
    rather than <= / >=.
  - If, after excluding the current candle, no other candles remain
    in the window (e.g. very early in the buffer's life), there is
    no prior extreme to compare against and the check is skipped for
    that minute, exactly as when the buffer has no data at all.

This affects trigger evaluation only. It does not change how the
reference low/high is displayed on charts (chart threshold lines
still reflect the standard rolling low/high over the full window,
current candle included, for visual continuity with the plotted
candles).
...
contract size, contract count, or price.
  ...
  - INITIALIZATION: on process startup, for every symbol that does
    ...
    startup step.
  - UPDATE RULE: every time a symbol TRIGGERS (strict new-low or
    new-high condition satisfied — see STRICT NEW-LOW / NEW-HIGH
    SEMANTICS — for BOTH failed and non-failed symbols), BudgetR is
    updated as follows, BEFORE the order-sizing formula is
    evaluated for that same trigger:
      - Let previous_trigger_price be the candle extreme (low for
        mode 1, high for mode 2) recorded at this symbol's MOST
        RECENT PRIOR TRIGGER (i.e. the most recent prior minute at
        which the strict new-extreme condition was itself satisfied
        for this symbol — NOT merely the most recently checked
        candle, which triggers every minute regardless of outcome).
      - If there is no previous_trigger_price on record yet (this is
        the symbol's first-ever trigger), OR if the current candle's
        extreme did NOT set a new trigger-extreme relative to the
        last trigger (mode 1: candle_low >= previous_trigger_price;
        mode 2: candle_high <= previous_trigger_price):
            BudgetR[sym] <- budget[sym]   (reset to the LIVE running
                                            budget at this moment)
      - Otherwise (mode 1: candle_low < previous_trigger_price;
        mode 2: candle_high > previous_trigger_price, i.e. this
        trigger IS itself a new extreme relative to the previous
        trigger):
            BudgetR[sym] is left unchanged.
      - In either case, this trigger's candle extreme is then
        recorded as the new previous_trigger_price for the NEXT
...
the pending accumulator, computed as:
    ...
    order_size_usd = contribution + BudgetR / 1000
...
1000)) * contribution": expanding that product algebraically
cancels one power of contribution against the contribution*1000
term in the denominator, leaving exactly contribution +
BudgetR/1000 with no division by contribution anywhere in the
final expression. contribution therefore never appears as a
divisor in this formula, and no divide-by-zero guard is needed for
it here. contribution's only role in the final formula is as an
additive floor: when BudgetR is 0, order_size_usd reduces to
exactly contribution, matching the pre-scaling per-trigger
baseline.
...
credited with:
    ...
    accrual_usd = BUDGET_DAILY_ACCRUAL_MULT * contrib_per_trigger_usd(sym)
...
tracking:
  ...
  - Trigger conditions are still evaluated every minute purely for
    ...
    chart. Because failed symbols never place orders, they never
...
OHLC candles (~960 candles). The chart marks:
  - Whichever reference extreme is currently active (2d or 9d, per
    budget sign; low for mode 1, high for mode 2) as a dashed
    horizontal threshold line. For failed symbols (budget frozen at
    0 or whatever it was at time of failure), this still reflects
    budget sign the same way.
  - Every TRIGGER (strict new extreme versus the prior-window
    reference, excluding the current candle) as a small tick
...
each abbreviated column header to its full name. Columns, in order:
  ...
  Sym     Symbol code. A failed symbol has "[F]" appended directly
          ...
          color the way the former red/blue/orange scheme did. A
          mode-2 symbol has " (2)" appended after its code so the
          execution mode is visible at a glance. In the Totals row,
          this cell reads "Totals" instead of a symbol code.
...
will be nonzero if price action crossed their reference extreme;
their order counts will always be 0/0 since no orders are ever
attempted) so the report reflects that they're being watched but
not traded.
...
═══════════════════════════════════════════════════════════════════
  ...
  - On startup: run a one-time TEST order (limit LONG at market-10%
    for mode 1, or limit SHORT at market+10% for mode 2; sized at
    that symbol's own exchange-reported minimum order size) against
    EVERY symbol, in three flat batch phases (no threads):
      1. OPEN  — send a test order for every symbol.
...
#
# SYMBOLS: the list of actively-monitored symbols.
#
# SYMBOL_MODES: execution mode per symbol (1 or 2 — see SYMBOL
#   EXECUTION MODE in the module docstring). Every symbol in SYMBOLS
#   should have an entry here; a symbol with no entry defaults to
#   MODE 1 (the original long/buy-the-dip behavior).
...
# Mode 1 = long / buy-the-dip (trigger on new lows, open limit LONG).
# Mode 2 = close / sell-the-rip (trigger on new highs, open limit SHORT
#          to close long exposure; no budget deduction on success).
SYMBOL_MODES: Dict[str, int] = {
    "USOIL_USDT":   1,
    "URNM_USDT":    2,
    "BTC_USDT":     2,
    "ETH_USDT":     2,
...
def get_mode(sym: str) -> int:
    """Returns the execution mode for sym (1 or 2), defaulting to 1
    for any symbol with no explicit entry in SYMBOL_MODES."""
    return SYMBOL_MODES.get(sym, 1)


def is_close_mode(sym: str) -> bool:
    """True for mode-2 (close / sell-the-rip) symbols."""
    return get_mode(sym) == 2
...
ROLL_MINUTES_SHORT = 2 * 24 * 60     # 2 days, in minutes -> "2d low/high"
ROLL_MINUTES_LONG  = 9 * 24 * 60     # 9 days, in minutes -> "9d low/high"
...
def init_budget_r_if_absent(sym: str):
    ...
    but ONLY if this symbol has no BudgetR entry at all yet."""
...
def update_budget_r_on_trigger(sym: str, candle_extreme: float) -> float:
    """Applies the BUDGET-R update rule for a single trigger and
    returns the resulting BudgetR value (post-update) for immediate
    use in the order-sizing formula. Also records this trigger's
    candle_extreme as the new last_trigger_low for the NEXT
    trigger's comparison, and increments this symbol's lifetime
    trigger count.

    Note: 'candle_extreme' is the candle LOW for mode-1 symbols and
    the candle HIGH for mode-2 symbols. The comparison direction
    follows the symbol's mode so that "a new trigger-extreme" means
    the same thing within each mode.
    """
    close_mode = is_close_mode(sym)

    with _STATE_DATA_LOCK:
        prev_extreme = get_last_trigger_low(sym)

        if close_mode:
            # Mode 2: trigger-extreme is a HIGH; "new extreme" means
            # strictly HIGHER than the previous trigger's high.
            is_new_trigger_extreme = (
                prev_extreme is not None and candle_extreme > prev_extreme
            )
        else:
            # Mode 1: trigger-extreme is a LOW; "new extreme" means
            # strictly LOWER than the previous trigger's low.
            is_new_trigger_extreme = (
                prev_extreme is not None and candle_extreme < prev_extreme
            )

        if is_new_trigger_extreme:
            new_budget_r = get_budget_r(sym)
            log.info(
                f"[{sym}] BudgetR unchanged (this trigger extreme "
                f"{candle_extreme:.4f} is a new "
                f"{'high' if close_mode else 'low'} vs previous "
                f"trigger extreme {prev_extreme:.4f}): "
            ...
            reason = (
                ...
                if prev_extreme is None
                    ...
                    f"this trigger extreme {candle_extreme:.4f} did not set "
                    f"a new trigger-extreme vs previous "
                    f"{prev_extreme:.4f}"
                )
            )
            log.info(
        ...
        STATE_DATA["last_trigger_low"][sym] = candle_extreme
...
def compute_order_size_usd(sym: str, budget_r: float) -> float:
    ...
    "(1 + BudgetR/(contribution*1000)) * contribution".
    """
...
def executed_orders_for_sym(sym: str) -> List[Dict]:
    ...
    identified by carrying a 'reference_window' key."""
    return [
...
def _effective_leverage(sym: str) -> int:
    ...
    maxLeverage for that contract if one is known."""
...
def place_long(
    ...
    return _place_contracts(sym, limit_price, vol, side=1)
...
def place_short(
    ...
    """Places a limit SHORT order — used by mode-2 (close) symbols
    to unwind long exposure on a new-high trigger."""
    vol = _contracts(sym, usd_amount, sizing_price)
    ...
    return _place_contracts(sym, limit_price, vol, side=3)
...
def place_short_min_size(sym: str, limit_price: float) -> Optional[str]:
    vol = _mos(sym)
    return _place_contracts(sym, limit_price, vol, side=3)


def _place_contracts(
    sym: str,
    limit_price: float,
    vol: float,
    side: int
    ...
    if not r.get("success"):
        log.error(f"[{sym}] order rejected: {r}")
        return None
...
class MinuteBuffer:
    ...
    def rolling_high(
        ...
        """Returns the maximum high across closed candles within the
        trailing window_minutes. Mirror of rolling_low, used by
        mode-2 (close / sell-the-rip) symbols to find new highs."""
        with self.lock:
            if not self.bars:
                return None
            ...
            window = [b["h"] for b in bars if b["t"] >= cutoff]
...
def seed_minute_buffer(sym: str):
    ...
    if span_days < (BUFFER_MAX_MINUTES / 1440.0) * 0.9:
        log.warning(
            ...
            "9d reference and 10d chart will be based on "
...
def evaluate_trigger(sym: str, now_utc: datetime.datetime):
    ...
    if close_mode:
        candle_extreme = latest["h"]
        # exclude_latest=True: the reference high must reflect only
        # candles PRIOR to the one currently under evaluation, so
        # that a trigger requires a genuine new high relative to
        # the rest of the window rather than the current candle
        # trivially matching itself as the window maximum.
        ref_extreme = buf.rolling_high(ref_window, exclude_latest=True)

        if ref_extreme is None:
            log.warning(
                f"[{sym}] insufficient prior-candle data to compute "
                f"{ref_label} high (excluding current candle) — "
                "skipping this minute"
            )
            return None

        # Strict inequality: the current candle's high must be
        # greater than every other high in the window.
        triggered = candle_extreme > ref_extreme
        extreme_label = "high"
    else:
        candle_extreme = latest["l"]
        ref_extreme = buf.rolling_low(ref_window, exclude_latest=True)
    ...
    log.info(
        ...
        f"{extreme_label}={candle_extreme:.4f} "
        f"{ref_label}{extreme_label.capitalize()}(prior)={ref_extreme:.4f} "
        f"budget={budget:.2f} "
        f"mode={get_mode(sym)} "
    ...
    return candle_dt, candle_extreme, ref_label, extreme_label, triggered
...
def process_symbol_minute(sym: str, now_utc: datetime.datetime):
    ...
    candle_dt, candle_extreme, ref_label, extreme_label, triggered = result

    if not triggered:
        return

    record_trigger_marker(sym, candle_dt, candle_extreme, ref_label)
    record_trigger_stat(sym)

    # BudgetR is updated for EVERY trigger, failed symbols included,
    # per the BUDGET-R rule. This must happen BEFORE the order-sizing
    # formula is evaluated, since the formula consumes the
    # POST-update BudgetR value.
    budget_r_now = update_budget_r_on_trigger(sym, candle_extreme)

    if failed:
        log.info(
            f"[{sym}] TRIGGER (failed symbol, marker-only, "
            f"no accumulation) @ price={candle_extreme:.4f} "
    ...
    log.info(
        ...
        f"@ price={candle_extreme:.4f}"
    )

    live_budget = get_budget(sym)

    # Negative budget safety stack (mode 1 only — mode 2 never
    # deducts budget, so a negative budget never blocks a close).
    if not close_mode and live_budget < 0:
        log.info(
    ...
    # Positive budget safety cap (mode 1 only)
    if not close_mode and live_budget >= 0 and pending > live_budget:
    ...
    vol_at_price = _contracts(sym, attempt_usd, candle_extreme)

    # Evaluate the exact cost equivalent for the fully-truncated contract volume
    cs = specs.get(sym, {}).get("cs", 1.0)
    actual_usd = vol_at_price * cs * candle_extreme

    if vol_at_price < _mos(sym):
        log.info(
            f"[{sym}] attempt size ${attempt_usd:.2f} (vol={vol_at_price}) below min order size "
            f"({_mos(sym)} contracts @ {candle_extreme:.4f}) — stacking, no order placed"
        )
        return

    side_label = "SHORT (close)" if close_mode else "LONG"

    log.info(
        f"[{sym}] accumulator attempt ${attempt_usd:.2f} reaches min order "
        f"size — attempting limit {side_label} {vol_at_price} contracts "
        f"@ {candle_extreme:.4f} "
        f"(actual cost: ${actual_usd:.2f})"
    )

    if close_mode:
        oid = place_short(sym, candle_extreme, candle_extreme, attempt_usd)
    else:
        oid = place_long(sym, candle_extreme, candle_extreme, attempt_usd)

    if oid == "SKIP" or oid is None:
        record_attempt_stat(sym, candle_extreme, success=False)
        ...
        if oid == "SKIP":
            log.warning(
                f"[{sym}] fire skipped by place_* despite "
    ...
    record_attempt_stat(sym, candle_extreme, success=True, usd_if_success=actual_usd)
    record_lifetime_order_outcome(sym, success=True)

    deduct_accumulator(sym, actual_usd)

    # Mode 1 (long / buy-the-dip) consumes budget on success.
    # Mode 2 (close / sell-the-rip) does NOT — closing long
    # exposure is the unwind side and does not use up the dip-
    # buying budget. See SYMBOL EXECUTION MODE in the module
    # docstring.
    if close_mode:
        log.info(
            f"[{sym}] close-mode fill — budget NOT deducted "
            f"(budget unchanged at ${live_budget:.2f})"
        )
    else:
...
def build_daily_report_text(now_utc: datetime.datetime) -> str:
    """Builds the plain-text daily activity report body."""
    window_start = None
...
def _open_test_order(sym: str) -> Optional[Dict]:
    ...
    try:
        ...
        # Mode 1: limit LONG 10% below market (a passive buy).
        # Mode 2: limit SHORT 10% above market (a passive close),
        # which is the exact mirror.
        if close_mode:
            test_price = mark * (2.0 - TEST_ORDER_DISCOUNT)  # 1.10 * mark
            side_label = "SHORT"
        else:
        ...
        log.info(
            f"[{sym}] test order OPEN ({side_label}): "
            f"mark={mark:.4f} "
        ...
        if close_mode:
            oid = place_short_min_size(sym, test_price)
        else:
    ...
    try:
        if is_filled(sym, oid):
            log.warning(
                ...
                "This is now a real open position. Symbol remains validated."
            )
            record_order({
                "symbol": sym,
...
def _total_exposure_usd(sym: str) -> float:
    ...
    order for this symbol, lifetime."""
    orders = executed_orders_for_sym(sym)
...
def render_overview_table_html(now_utc: datetime.datetime) -> str:
    ...
    string, plus the light-gray legend subtext placed BELOW it."""
    ...
    # Totals footer row: populated only for columns in TOTALS_COLUMNS.
...
def render_symbol_chart_svg(sym: str) -> str:
    ...
    if close_mode:
        ref_extreme = buf.rolling_high(ref_window)
        ref_extreme_label = "high"
    else:
        ref_extreme = buf.rolling_low(ref_window)
        ref_extreme_label = "low"

    lo = min(c["l"] for c in candles)
    hi = max(c["h"] for c in candles)

    if ref_extreme is not None:
        lo = min(lo, ref_extreme)
        hi = max(hi, ref_extreme)
    ...
    mode_label = " [MODE 2 — close/sell-the-rip]" if close_mode else ""
    ...
    if ref_extreme is not None:
        ...
        svg.append(
            ...
            f'{_xml_escape(f"{ref_label} {ref_extreme_label} threshold: {ref_extreme:,.4f}")}</text>'
        )
    ...
    for o in orders:
        ...
        if is_real_fire:
            # Real fires: blue for mode 1 (long), orange for mode 2 (close)
            marker_color = "#ff8c00" if o.get("mode") == 2 else "#0044cc"
        else:
            marker_color = "#888"
    ...
    svg.append(
        ...
        f'font-family="Courier New" font-size="10" fill="#555">'
        f'order placed (long)</text>'
    )

    svg.append(
        f'<circle cx="{CHART_MARGIN_L + 240}" cy="{legend_y}" r="4" '
        f'fill="#ff8c00" stroke="#fff" stroke-width="1"/>'
    )
    svg.append(
        f'<text x="{CHART_MARGIN_L + 250}" y="{legend_y + 4}" '
        f'font-family="Courier New" font-size="10" fill="#555">'
        f'order placed (close)</text>'
    )
...
def engine_cycle():
    ...
    # This is the ONLY status computation in the entire bot.
    STATE.set_status(
        f"ok  "
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/chart/") and self.path.endswith(".svg"):
            ...
            body = json.dumps(
                {
                    ...
                    "modes": {sym: get_mode(sym) for sym in SYMBOLS},
                },
                indent=2
            ).encode("utf-8")
            ...
            # Exactly one status computation on the whole page.
